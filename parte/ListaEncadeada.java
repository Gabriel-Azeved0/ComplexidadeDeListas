...
public class ListaEncadeada<T extends Comparable<T>> {
    ...
    public void inserirElemento(T elem) {
        if (this.contemElemento(elem)) {
            System.out.println("Elemento ja consta na lista.");
            return;
        }
        ...
        if(this.isOrdenada) {
            ...
            while (atual != null) {//condição que para ao chegar ao fim da lista
                result_comp = this.ordenador.compare(atual.getValor(), novo.getValor());
                if (result_comp > 0) {//ja passou pela posição que estaria o elemento
    ...
    public boolean contemElemento(T elem) {
        ...
        while (atual != null) {//condição que para ao chegar ao fim da lista
            result_comp = this.ordenador.compare(atual.getValor(), novo.getValor());
            if (result_comp == 0) {//Valor encontrado na lista
                return true;
            }
            if (result_comp > 0 && this.isOrdenada) {//caso seja ordenada e ja passou pela posição que estaria o elemento
                return false;
            }
            atual = atual.getProx();
        }
